...
WITH cte_Cohort_Items AS (							         -- To get users and their first engagements with the retail store 
	SELECT DATETRUNC(MONTH, MIN(Invoice_Date)) AS Cohort_First_Month 
	 	 , Customer_ID
		 , Country
	 FROM [AutoSales].[dbo].[Online_Retail_Store_Dataset]
...
, cte_Activities AS (								          -- Calculates when users have been sent comms relative to their cohort 
	SELECT a.Customer_ID
		, CASE WHEN DATEDIFF(MONTH,a.Invoice_Date,b.Cohort_First_Month) < 0 THEN -DATEDIFF(MONTH,a.Invoice_Date,b.Cohort_First_Month) ELSE DATEDIFF(MONTH,a.Invoice_Date,b.Cohort_First_Month) END AS Month_Number 
	 FROM [AutoSales].[dbo].[Online_Retail_Store_Dataset] a
	 LEFT JOIN cte_Cohort_Items b 
	   ON a.Customer_ID = b.Customer_ID 
    WHERE Invoice_Date IS NOT NULL 
    GROUP BY a.Customer_ID, a.Invoice_Date, b.Cohort_First_Month
)

, cte_Cohort_Size AS (								         -- Counts the number of users in each cohort month 
...
, cte_Retention_Table AS (							     	 -- Calculates how many users are active in subsequent months  
  ...
  SELECT  t.cohort_First_month																		-- Cohort month was formed     
		, t.month_number																	-- Number of months after the first month    
		, s.num_users AS total_users																-- Total number of users in the cohort   
		, t.num_users AS users_from_first_month															-- Users still getting comms from the first month     
